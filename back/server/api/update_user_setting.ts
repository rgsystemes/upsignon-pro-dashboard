...
export const update_user_setting = async (req: any, res: any): Promise<void> => {
  try {
    ...
    if (Object.keys(req.body).indexOf('allowed_offline_desktop') != -1) {
      await db.query(`UPDATE users SET allowed_offline_desktop=$1 WHERE id=$2 AND bank_id=$3`, [
    ...
    if (Object.keys(req.body).indexOf('allowed_offline_mobile') != -1) {
      await db.query(`UPDATE users SET allowed_offline_mobile=$1 WHERE id=$2 AND bank_id=$3`, [
    ...
    if (Object.keys(req.body).indexOf('allowed_to_export') != -1) {
      await db.query(`UPDATE users SET allowed_to_export=$1 WHERE id=$2 AND bank_id=$3`, [
    ...
    if (Object.keys(req.body).indexOf('settings_override') != -1) {
      {
        await db.query(`UPDATE users SET settings_override=$1 WHERE id=$2 AND bank_id=$3`, [
