...
// This mirrors upsignon-pro-server's src/helpers/licenceCheck.ts hasAvailableLicence so that
// unarchiving a user here consumes the exact same licence pool (bank -> reseller pool -> superadmin pool)
// that the vault app itself is checked against when a new vault is created.
